...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        for course, prereq in prerequisites:
            ...
            if prereq not in courseMap:
                courseMap[prereq] = set()
        ...
        def dfsHelper(node):
            ...
            for neighbor in courseMap[node]:
                if dfsHelper(neighbor) == "Contradiction":
                    coursePossibility[node] = "Contradiction"
                    return "Contradiction"
            
            visited.remove(node)

            coursePossibility[node] = "Possible"
            return "Possible"

        for course in courseMap:            
            if dfsHelper(course) == "Contradiction":
                return False
